const modeloListaAmigos = require('./modeloListaA')

//Exportamos nuestros Modulos

module.exports.obtainFriends = async (amigo) => {
  try {
    const amigos = await modeloListaAmigos.buscarAmigos()
    return amigos
  } catch (err) {
...
module.exports.addAmigo = async (amigo) => {
  try {
    const amigoNuevo = await modeloListaAmigos.crearAmigo(amigo)
...
module.exports.deleteFriend = async (idAmigo)=> {
  try {
      let result = await modeloListaAmigos.borrarAmigo(idAmigo)
      return result
  }catch (error) {
